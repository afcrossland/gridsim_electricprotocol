...
interface Props {
  scenes: Scene[];
  onDismiss: () => void;
  /** Reports the active scene's id back up - lets the host `App.tsx` know when the opening hero scene is showing, so it can hide its own sidebar/nav/footer for an unobstructed view underneath, same as every app that used this before it was shared. */
  onSceneChange?: (sceneId: number) => void;
  /** Plain strings for the nav/hint UI - override any subset; anything omitted falls back to the English default (no app is forced to wire up i18n just to use this). */
...
/**
 * The shared tour engine - wheel/keyboard-driven scene navigation, the
 * spotlight mechanic, progress dots, nav buttons, and a Skip/"Start
 * exploring" pill. Generalized 2026-09-19 from deployment's own
 * `ScrollStory.tsx` (itself "a trimmed sibling" of the root app's fuller
 * version - see this file's own history) into `shared/tour/` - a `Scene`
 * only needs `onEnter` for any side effect it wants (deployment's own
 * "select this demo country" case is just one example, not baked in
 * here), and the small amount of UI text is passed in via `labels`/
 * `heroFooter` rather than assuming `react-i18next` exists (calculator,
 * the newest app to use this, has no i18n at all).
 *
...
export default function TourOverlay({ scenes, onDismiss, onSceneChange, labels, heroFooter }: Props) {
  ...
  const goToScene = useCallback((index: number) => setActiveScene(Math.max(0, Math.min(index, scenes.length - 1))), [scenes.length]);
    ...
    <Box sx={{ position: "fixed", inset: 0, zIndex: 300, overflow: "hidden", pointerEvents: "none" }}>
        ...
        {scenes.map((_, i) => (
          <Box
            ...
            sx={{
              ...
              borderRadius: "3px",
              cursor: "pointer",
              bgcolor: i === activeScene ? RING : i < activeScene ? "rgba(0,171,187,0.55)" : "rgba(120,130,140,0.4)",
      ...
      >
        ...
        <Box sx={{ display: "flex", gap: 1.5, alignItems: "center" }}>
          <Box component="button" onClick={retreat} disabled={activeScene === 0} aria-label={L.previous} sx={NAV_BTN}>
          ...
          >
            {activeScene === scenes.length - 1 ? L.startExploring : L.skipIntro}
          </Box>
          <Box
            component="button"
            onClick={advance}
